...
const findSleepiestGuard = data => {
    ...
    const bestGuard = guardDataWithMaxSleepTimeCount.reduce((previousGuard, guard) => (guard.maxTimeAsleep < previousGuard.maxTimeAsleep) ? previousGuard : guard);
    return bestGuard;
}

// const sleepiestGuard = findSleepiestGuard(testData);
const sleepiestGuard = findSleepiestGuard(readData('testData.txt'));

console.log('Best guard id multiplied by minute', sleepiestGuard.guardID * sleepiestGuard.maxTimeCount[0]);